const Cat_OrgStruccture = require("./Cat_OrgStruccture.model");
const qs = require("qs");
...
exports.get = async (req, res, next) => {
  try {
    ...
    const data = await Cat_OrgStruccture.find(filters)
      .sort(sort)
      .select(fields)
      .skip((page - 1) * perPage)
      .limit(perPage);

    const totalDocuments = await Cat_OrgStruccture.find(filters)
      .sort(sort)
      .countDocuments();
